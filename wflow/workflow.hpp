...
public:
  ...
  /**
   * Конструктор без поддержки монопоточного режима.
   * В конфигурации необходимо указать threads=1 или больше 
   * При threads задачи из очереди обрабатываться не будут
   * 
   * @param opt - опции. 
   */
   ...
   * 
   * При threads=1 или большем создается свой объект io_service или аналогичный, а io не используется 
   */
  ...
  /**
   * Отправить заданиe на обработку
  ...
  /**
   * Отправить заданиe на обработку в указанный момент времени
   * @param tp момент времени, в который задание будет отправленно на обработку
   * @param handler обработчик задания типа void()
   * @param drop альтернативный обработчик, если задание выкидывается из очереди. По умолчанию nullptr (handler выкидывается из очереди без уведомления)
   ...
   * в отличии от safe_post, где оно будет выполненно сразу.
   */
  bool post(time_point_t tp, post_handler handler, drop_handler drop = nullptr);
  
  
  /**
   * Отправить заданиe на обработку через указанный интервал времени
   * @param duration интервал времени, через который задание будет отправленно на обработку
   * @param handler обработчик задания типа void()
   * @param drop альтернативный обработчик, если задание выкидывается из очереди. По умолчанию nullptr (handler выкидывается из очереди без уведомления)
   * @return true - включен таймер для отправки сообщения. Таймер выкинут из очереди быть не может поэтому всегда true
   * 
  ...
  /**
   * Отправить заданиe на обработку игнорируя ограничения на размер очереди и скорости обработки.
   * @param handler обработчик задания типа void()
   * 
  ...
  /**
   * Выполнить заданиe в указанный момент времени
   * @param tp момент времени, в который задание будет выполненно
   * @param handler обработчик задания типа void()
   */
  void safe_post(time_point_t tp, post_handler handler);

  /**
   * Выполнить заданиe через указанный интервал времени
   * @param duration интервал времени, через который задание будет выполненно
   * @param handler обработчик задания типа void()
   */
   ...
   * @param d интервал таймера 
   * @param h обработчик таймера std::function<bool()>, который должен возвращать true, если таймер должен продолжать работать и false - для завершения.
   ...
   */
  timer_id_t create_timer(duration_t d, timer_handler h, expires_at expires = expires_at::after);
   ...
   * @param d интервал таймера 
   * @param ah обработчик таймера std::function<void(callback_timer_handler)>, который должен вызвать callback_timer_handler(true), 
   ...
   * @return идентификатор таймера, который можно использовать для остановки таймера
   */
  timer_id_t create_async_timer(duration_t d, async_timer_handler ah, expires_at expires = expires_at::after);

  
  /**
   * Создает таймер, обработчик которого выполняется с заданной периодичностью через определенный интервал времени 
   * @param ds интервал времени, через который запустить таймер
   * @param d интервал таймера 
   * @param h обработчик таймера std::function<bool()>, который должен возвращать true, если таймер должен продолжать работать и false - для завершения.
   * @param expires если expires_at::after (по умолчанию), то отсчет до следующего запуска после выполнения обработчика таймера
   * 
   * @return идентификатор таймера, который можно использовать для остановки таймера
   */
  timer_id_t create_timer(duration_t ds, duration_t d, timer_handler h, expires_at expires = expires_at::after);
   ...
   * @return идентификатор таймера, который можно использовать для остановки таймера
   */
  timer_id_t create_async_timer(duration_t ds, duration_t d, async_timer_handler ah, expires_at expires = expires_at::after);
   ...
   * @return идентификатор таймера, который можно использовать для остановки таймера
   */
  timer_id_t create_timer(time_point_t tp, duration_t d, timer_handler h, expires_at expires = expires_at::after);
   ...
   * @return идентификатор таймера, который можно использовать для остановки таймера
   */
  timer_id_t create_async_timer(time_point_t tp, duration_t d, async_timer_handler ah, expires_at expires = expires_at::after);
   ...
   * @return идентификатор таймера, который можно использовать для остановки таймера
   */
  timer_id_t create_timer(std::string tp, duration_t d, timer_handler h, expires_at expires = expires_at::after);
  
  timer_id_t create_async_timer(std::string tp, duration_t d, async_timer_handler ah, expires_at expires = expires_at::after);


  timer_id_t create_timer(std::string tp, timer_handler h, expires_at expires = expires_at::after);
  timer_id_t create_async_timer(std::string tp, async_timer_handler ah, expires_at expires = expires_at::after);
  ...
  /**
   * Отсоеденить таймер. Таймер продолжает работать до тех пор пока существует объект, который вернул detach_timer
   * @param id - идентификатор таймера
