...
``` 
   ...
   Если не задано ограничений на размер очереди (по умолчанию), то задания не будут выкидываться из очереди, но отработка отложеных заданий и/или при ограничении скорости обработки
   отличается. В то время как у защищенного отложенного задания таймер устанавливается на время выполнения обработчика задания, то у незащенного задания - на время отправки в общую 
   очередь. Т.е. задание на отправку в очередь, в заданное время, будет выполнено, но не факт, что успешно, если очередь будет переполненна. \ref example04.cpp 
   ...
   если тамер еще актуален и false - если он больше не нужен. Помимо интервала выполнения, можно передать флаг определяющий, как будет отсчитываться следующий момент времени вызова 
