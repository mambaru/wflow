   ...
   namespace detail
   {
         ...
         cron_int const maxval)
      {
         ...
         for (auto const & field : fields)
         {
...
#endif
               for (cron_int i = static_cast<cron_int>(first - minval); i <= static_cast<cron_int>(last - minval); ++i)
               {
                  target.set(i);
               }
            }
            else 
            {
               ...
               for (cron_int i = static_cast<cron_int>(first - minval); i <= static_cast<cron_int>(last - minval); i = static_cast<cron_int>(i + delta))
               {
                  target.set(i);
               }
            }
         ...
         std::bitset<7> const & marked_fields)
      {
            ...
            && count++ < maximum)
         {
            ...
            day_of_month = static_cast<cron_int>(date.tm_mday);
            day_of_week = static_cast<cron_int>(date.tm_wday);
                            ...
                            size_t const dot)
      {
         ...
         {
            if ( static_cast<size_t>(date.tm_year) - dot > static_cast<size_t>(Traits::CRON_MAX_YEARS_DIFF) )
               return false;
